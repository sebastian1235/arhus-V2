    ...
    public function validarModoPagoController($validarMedioPago){
     ...
     public function eliminarModelController(){
        ...
        if (isset($_POST["eliminarMedioPago"])) {
            $datosController = array("Id_medio_pago"=> $_POST["idMedioPago"],
                                      "eliminar" => $_POST["eliminarMedioPago"]
                                     );
            $respuesta = MedioPagoModel::EliminarMedioPago($datosController, "siax_medio_pago");

            if ($respuesta == "ok") {
                if(isset($_POST["actualizarSesion"])){
                    $_SESSION["Id_medio_pago"] = $_POST["idMedioPago"];
                    $_SESSION["eliminar"] = $_POST["eliminarMedioPago"];
                

                }
                if ($_POST["eliminarMedioPago"] == 0) {
                 echo '<script>
                                swal({
                                  title: "!No",
