...
#Vista Asigancion
    ...
    {
        $stmt = Conexion::conectar()->prepare("SELECT id_sol, poliza_sol, consecutivo_cot, nombre_forma_ap, nombre_campana, nombre_sol FROM $tabla left join ap_forma_pago on ap_solicitud.forma_pago_cot= ap_forma_pago.Id_forma_ap LEFT JOIN siax_campana on ap_solicitud.campana_cot=siax_campana.id_campana where eliminar='0'");
        $stmt->execute();
        return $stmt->fetchAll();
        $stmt->close();
    }
