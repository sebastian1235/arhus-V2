...
$ap_asignacion_add = NULL; // Initialize page object first

class cap_asignacion_add extends cap_asignacion {
	...
	// Page object name
	var $PageObjName = 'ap_asignacion_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_asignacion"] != "") {
				...
				$this->setKey("id_asignacion", $this->id_asignacion->CurrentValue); // Set up key
			} else {
				$this->setKey("id_asignacion", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "ap_asignacionview.php")
	...
	function LoadDefaultValues() {
		$this->tipo_asignacion->CurrentValue = NULL;
		$this->tipo_asignacion->OldValue = $this->tipo_asignacion->CurrentValue;
		$this->comision_obra_asignacion->CurrentValue = 0.00000;
		$this->comision_gasod_asignacion->CurrentValue = 0.00000;
		$this->comision_fija_asignacion->CurrentValue = NULL;
		$this->comision_fija_asignacion->OldValue = $this->comision_fija_asignacion->CurrentValue;
		$this->empresa_asignacion->CurrentValue = NULL;
		$this->empresa_asignacion->OldValue = $this->empresa_asignacion->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// tipo_asignacion
		$this->tipo_asignacion->SetDbValueDef($rsnew, $this->tipo_asignacion->CurrentValue, NULL, FALSE);

		// comision_obra_asignacion
		$this->comision_obra_asignacion->SetDbValueDef($rsnew, $this->comision_obra_asignacion->CurrentValue, NULL, strval($this->comision_obra_asignacion->CurrentValue) == "");

		// comision_gasod_asignacion
		$this->comision_gasod_asignacion->SetDbValueDef($rsnew, $this->comision_gasod_asignacion->CurrentValue, NULL, strval($this->comision_gasod_asignacion->CurrentValue) == "");

		// comision_fija_asignacion
		$this->comision_fija_asignacion->SetDbValueDef($rsnew, $this->comision_fija_asignacion->CurrentValue, NULL, FALSE);

		// empresa_asignacion
		$this->empresa_asignacion->SetDbValueDef($rsnew, $this->empresa_asignacion->CurrentValue, NULL, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->id_asignacion->setDbValue($conn->Insert_ID());
				$rsnew['id_asignacion'] = $this->id_asignacion->DbValue;
			}
		} else {
...
// Create page object
if (!isset($ap_asignacion_add)) $ap_asignacion_add = new cap_asignacion_add();

// Page init
$ap_asignacion_add->Page_Init();

// Page main
$ap_asignacion_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ap_asignacion_add->Page_Render();
?>
...
var CurrentForm = fap_asignacionadd = new ew_Form("fap_asignacionadd", "add");

// Validate form
fap_asignacionadd.Validate = function() {
...
fap_asignacionadd.Form_CustomValidate = 
...
fap_asignacionadd.ValidateRequired = true;
<?php } else { ?>
fap_asignacionadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$ap_asignacion_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $ap_asignacion_add->ShowPageHeader(); ?>
<?php
$ap_asignacion_add->ShowMessage();
?>
<form name="fap_asignacionadd" id="fap_asignacionadd" class="<?php echo $ap_asignacion_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($ap_asignacion_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $ap_asignacion_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $ap_asignacion_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fap_asignacionadd.Init();
</script>
<?php
$ap_asignacion_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ap_asignacion_add->Page_Terminate();
?>
