...
$ap_detalle_venta_add = NULL; // Initialize page object first

class cap_detalle_venta_add extends cap_detalle_venta {
	...
	// Page object name
	var $PageObjName = 'ap_detalle_venta_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_det_venta"] != "") {
				...
				$this->setKey("id_det_venta", $this->id_det_venta->CurrentValue); // Set up key
			} else {
				$this->setKey("id_det_venta", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "ap_detalle_ventaview.php")
	...
	function LoadDefaultValues() {
		$this->venta_det_venta->CurrentValue = NULL;
		$this->venta_det_venta->OldValue = $this->venta_det_venta->CurrentValue;
		$this->codigo_det_venta->CurrentValue = NULL;
		$this->codigo_det_venta->OldValue = $this->codigo_det_venta->CurrentValue;
		$this->descripcion_det_venta->CurrentValue = NULL;
		$this->descripcion_det_venta->OldValue = $this->descripcion_det_venta->CurrentValue;
		$this->precio_det_venta->CurrentValue = NULL;
		$this->precio_det_venta->OldValue = $this->precio_det_venta->CurrentValue;
		$this->cantidad_det_venta->CurrentValue = 0.00000;
		$this->total_det_venta->CurrentValue = NULL;
		$this->total_det_venta->OldValue = $this->total_det_venta->CurrentValue;
		$this->tipo_det_venta->CurrentValue = 5;
		$this->empresa_det_venta->CurrentValue = NULL;
		$this->empresa_det_venta->OldValue = $this->empresa_det_venta->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// venta_det_venta
		$this->venta_det_venta->SetDbValueDef($rsnew, $this->venta_det_venta->CurrentValue, NULL, FALSE);

		// codigo_det_venta
		$this->codigo_det_venta->SetDbValueDef($rsnew, $this->codigo_det_venta->CurrentValue, NULL, FALSE);

		// descripcion_det_venta
		$this->descripcion_det_venta->SetDbValueDef($rsnew, $this->descripcion_det_venta->CurrentValue, NULL, FALSE);

		// precio_det_venta
		$this->precio_det_venta->SetDbValueDef($rsnew, $this->precio_det_venta->CurrentValue, NULL, FALSE);

		// cantidad_det_venta
		$this->cantidad_det_venta->SetDbValueDef($rsnew, $this->cantidad_det_venta->CurrentValue, NULL, strval($this->cantidad_det_venta->CurrentValue) == "");

		// total_det_venta
		$this->total_det_venta->SetDbValueDef($rsnew, $this->total_det_venta->CurrentValue, NULL, FALSE);

		// tipo_det_venta
		$this->tipo_det_venta->SetDbValueDef($rsnew, $this->tipo_det_venta->CurrentValue, NULL, strval($this->tipo_det_venta->CurrentValue) == "");

		// empresa_det_venta
		$this->empresa_det_venta->SetDbValueDef($rsnew, $this->empresa_det_venta->CurrentValue, NULL, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->id_det_venta->setDbValue($conn->Insert_ID());
				$rsnew['id_det_venta'] = $this->id_det_venta->DbValue;
			}
		} else {
...
// Create page object
if (!isset($ap_detalle_venta_add)) $ap_detalle_venta_add = new cap_detalle_venta_add();

// Page init
$ap_detalle_venta_add->Page_Init();

// Page main
$ap_detalle_venta_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ap_detalle_venta_add->Page_Render();
?>
...
var CurrentForm = fap_detalle_ventaadd = new ew_Form("fap_detalle_ventaadd", "add");

// Validate form
fap_detalle_ventaadd.Validate = function() {
...
fap_detalle_ventaadd.Form_CustomValidate = 
...
fap_detalle_ventaadd.ValidateRequired = true;
<?php } else { ?>
fap_detalle_ventaadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$ap_detalle_venta_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $ap_detalle_venta_add->ShowPageHeader(); ?>
<?php
$ap_detalle_venta_add->ShowMessage();
?>
<form name="fap_detalle_ventaadd" id="fap_detalle_ventaadd" class="<?php echo $ap_detalle_venta_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($ap_detalle_venta_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $ap_detalle_venta_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $ap_detalle_venta_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fap_detalle_ventaadd.Init();
</script>
<?php
$ap_detalle_venta_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ap_detalle_venta_add->Page_Terminate();
?>
