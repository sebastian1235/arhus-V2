...
$ap_estado_interno_view = NULL; // Initialize page object first

class cap_estado_interno_view extends cap_estado_interno {
	...
	// Page object name
	var $PageObjName = 'ap_estado_interno_view';
	...
	function __construct() {
		...
		if (@$_GET["id_estado_interno"] <> "") {
			$this->RecKey["id_estado_interno"] = $_GET["id_estado_interno"];
			$KeyUrl .= "&amp;id_estado_interno=" . urlencode($this->RecKey["id_estado_interno"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id_estado_interno"] <> "") {
				...
				$this->RecKey["id_estado_interno"] = $this->id_estado_interno->QueryStringValue;
			} elseif (@$_POST["id_estado_interno"] <> "") {
				$this->id_estado_interno->setFormValue($_POST["id_estado_interno"]);
				$this->RecKey["id_estado_interno"] = $this->id_estado_interno->FormValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("ap_estado_internolist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->id_estado_interno->CurrentValue) == strval($this->Recordset->fields('id_estado_interno'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "ap_estado_internolist.php"; // No matching record, return to list
					} else {
						$this->LoadRowValues($this->Recordset); // Load row values
					}
			}
		} else {
			$sReturnUrl = "ap_estado_internolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($ap_estado_interno_view)) $ap_estado_interno_view = new cap_estado_interno_view();

// Page init
$ap_estado_interno_view->Page_Init();

// Page main
$ap_estado_interno_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ap_estado_interno_view->Page_Render();
?>
...
var CurrentForm = fap_estado_internoview = new ew_Form("fap_estado_internoview", "view");

// Form_CustomValidate event
fap_estado_internoview.Form_CustomValidate = 
...
fap_estado_internoview.ValidateRequired = true;
<?php } else { ?>
fap_estado_internoview.ValidateRequired = false; 
<?php } ?>
...
<div class="ewToolbar">
<?php if (!$ap_estado_interno_view->IsModal) { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php $ap_estado_interno_view->ExportOptions->Render("body") ?>
<?php
	foreach ($ap_estado_interno_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$ap_estado_interno_view->IsModal) { ?>
...
<?php $ap_estado_interno_view->ShowPageHeader(); ?>
<?php
$ap_estado_interno_view->ShowMessage();
?>
<form name="fap_estado_internoview" id="fap_estado_internoview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($ap_estado_interno_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $ap_estado_interno_view->Token ?>">
<?php } ?>
...
<?php if ($ap_estado_interno->nombre_estado_interno->Visible) { // nombre_estado_interno ?>
	<tr id="r_nombre_estado_interno">
		<td><span id="elh_ap_estado_interno_nombre_estado_interno"><?php echo $ap_estado_interno->nombre_estado_interno->FldCaption() ?></span></td>
...
<?php echo $ap_estado_interno->nombre_estado_interno->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ap_estado_interno->para_factura_estado_interno->Visible) { // para_factura_estado_interno ?>
	<tr id="r_para_factura_estado_interno">
		<td><span id="elh_ap_estado_interno_para_factura_estado_interno"><?php echo $ap_estado_interno->para_factura_estado_interno->FldCaption() ?></span></td>
...
<?php echo $ap_estado_interno->para_factura_estado_interno->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ap_estado_interno->se_paga_comision_estado_interno->Visible) { // se_paga_comision_estado_interno ?>
	<tr id="r_se_paga_comision_estado_interno">
		<td><span id="elh_ap_estado_interno_se_paga_comision_estado_interno"><?php echo $ap_estado_interno->se_paga_comision_estado_interno->FldCaption() ?></span></td>
...
<?php echo $ap_estado_interno->se_paga_comision_estado_interno->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ap_estado_interno->porcen_comision_estado_interno->Visible) { // porcen_comision_estado_interno ?>
	<tr id="r_porcen_comision_estado_interno">
		<td><span id="elh_ap_estado_interno_porcen_comision_estado_interno"><?php echo $ap_estado_interno->porcen_comision_estado_interno->FldCaption() ?></span></td>
...
<?php echo $ap_estado_interno->porcen_comision_estado_interno->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ap_estado_interno->se_paga_bono_estado_interno->Visible) { // se_paga_bono_estado_interno ?>
	<tr id="r_se_paga_bono_estado_interno">
		<td><span id="elh_ap_estado_interno_se_paga_bono_estado_interno"><?php echo $ap_estado_interno->se_paga_bono_estado_interno->FldCaption() ?></span></td>
...
<?php echo $ap_estado_interno->se_paga_bono_estado_interno->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ap_estado_interno->porcen_bono_estado_interno->Visible) { // porcen_bono_estado_interno ?>
	<tr id="r_porcen_bono_estado_interno">
		<td><span id="elh_ap_estado_interno_porcen_bono_estado_interno"><?php echo $ap_estado_interno->porcen_bono_estado_interno->FldCaption() ?></span></td>
...
<?php echo $ap_estado_interno->porcen_bono_estado_interno->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ap_estado_interno->obs_estado_interno->Visible) { // obs_estado_interno ?>
	<tr id="r_obs_estado_interno">
		<td><span id="elh_ap_estado_interno_obs_estado_interno"><?php echo $ap_estado_interno->obs_estado_interno->FldCaption() ?></span></td>
		<td data-name="obs_estado_interno"<?php echo $ap_estado_interno->obs_estado_interno->CellAttributes() ?>>
<span id="el_ap_estado_interno_obs_estado_interno">
<span<?php echo $ap_estado_interno->obs_estado_interno->ViewAttributes() ?>>
<?php echo $ap_estado_interno->obs_estado_interno->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ap_estado_interno->envio_boffice_estado_interno->Visible) { // envio_boffice_estado_interno ?>
	<tr id="r_envio_boffice_estado_interno">
		<td><span id="elh_ap_estado_interno_envio_boffice_estado_interno"><?php echo $ap_estado_interno->envio_boffice_estado_interno->FldCaption() ?></span></td>
...
<?php echo $ap_estado_interno->envio_boffice_estado_interno->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ap_estado_interno->empresa_estado_interno->Visible) { // empresa_estado_interno ?>
	<tr id="r_empresa_estado_interno">
		<td><span id="elh_ap_estado_interno_empresa_estado_interno"><?php echo $ap_estado_interno->empresa_estado_interno->FldCaption() ?></span></td>
...
<?php echo $ap_estado_interno->empresa_estado_interno->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ap_estado_interno->id_estado_interno->Visible) { // id_estado_interno ?>
	<tr id="r_id_estado_interno">
		<td><span id="elh_ap_estado_interno_id_estado_interno"><?php echo $ap_estado_interno->id_estado_interno->FldCaption() ?></span></td>
...
<?php echo $ap_estado_interno->id_estado_interno->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if (!$ap_estado_interno_view->IsModal) { ?>
<?php if (!isset($ap_estado_interno_view->Pager)) $ap_estado_interno_view->Pager = new cPrevNextPager($ap_estado_interno_view->StartRec, $ap_estado_interno_view->DisplayRecs, $ap_estado_interno_view->TotalRecs) ?>
<?php if ($ap_estado_interno_view->Pager->RecordCount > 0 && $ap_estado_interno_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($ap_estado_interno_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $ap_estado_interno_view->PageUrl() ?>start=<?php echo $ap_estado_interno_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($ap_estado_interno_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $ap_estado_interno_view->PageUrl() ?>start=<?php echo $ap_estado_interno_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $ap_estado_interno_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($ap_estado_interno_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $ap_estado_interno_view->PageUrl() ?>start=<?php echo $ap_estado_interno_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($ap_estado_interno_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $ap_estado_interno_view->PageUrl() ?>start=<?php echo $ap_estado_interno_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $ap_estado_interno_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
<?php } ?>
...
<script type="text/javascript">
fap_estado_internoview.Init();
</script>
<?php
$ap_estado_interno_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ap_estado_interno_view->Page_Terminate();
?>
