...
$ap_items_inv_add = NULL; // Initialize page object first

class cap_items_inv_add extends cap_items_inv {
	...
	// Page object name
	var $PageObjName = 'ap_items_inv_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "ap_items_invview.php")
	...
	function LoadDefaultValues() {
		$this->codigo_item->CurrentValue = NULL;
		$this->codigo_item->OldValue = $this->codigo_item->CurrentValue;
		$this->nombre_item->CurrentValue = NULL;
		$this->nombre_item->OldValue = $this->nombre_item->CurrentValue;
		$this->und_item->CurrentValue = NULL;
		$this->und_item->OldValue = $this->und_item->CurrentValue;
		$this->precio_item->CurrentValue = 0.0000;
		$this->costo_item->CurrentValue = 0.0000;
		$this->tipo_item->CurrentValue = 6;
		$this->marca_item->CurrentValue = NULL;
		$this->marca_item->OldValue = $this->marca_item->CurrentValue;
		$this->cod_marca_item->CurrentValue = NULL;
		$this->cod_marca_item->OldValue = $this->cod_marca_item->CurrentValue;
		$this->detalle_item->CurrentValue = NULL;
		$this->detalle_item->OldValue = $this->detalle_item->CurrentValue;
		$this->saldo_item->CurrentValue = NULL;
		$this->saldo_item->OldValue = $this->saldo_item->CurrentValue;
		$this->activo_item->CurrentValue = 1;
		$this->maneja_serial_item->CurrentValue = 0;
		$this->asignado_item->CurrentValue = 0;
		$this->si_no_item->CurrentValue = 0;
		$this->precio_old_item->CurrentValue = NULL;
		$this->precio_old_item->OldValue = $this->precio_old_item->CurrentValue;
		$this->costo_old_item->CurrentValue = NULL;
		$this->costo_old_item->OldValue = $this->costo_old_item->CurrentValue;
		$this->registra_item->CurrentValue = NULL;
		$this->registra_item->OldValue = $this->registra_item->CurrentValue;
		$this->fecha_registro_item->CurrentValue = NULL;
		$this->fecha_registro_item->OldValue = $this->fecha_registro_item->CurrentValue;
		$this->empresa_item->CurrentValue = NULL;
		$this->empresa_item->OldValue = $this->empresa_item->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->codigo_item->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(codigo_item = '" . ew_AdjustSql($this->codigo_item->CurrentValue, $this->DBID) . "')";
		...
		// codigo_item
		$this->codigo_item->SetDbValueDef($rsnew, $this->codigo_item->CurrentValue, NULL, FALSE);

		// nombre_item
		$this->nombre_item->SetDbValueDef($rsnew, $this->nombre_item->CurrentValue, NULL, FALSE);

		// und_item
		$this->und_item->SetDbValueDef($rsnew, $this->und_item->CurrentValue, NULL, FALSE);

		// precio_item
		$this->precio_item->SetDbValueDef($rsnew, $this->precio_item->CurrentValue, NULL, strval($this->precio_item->CurrentValue) == "");

		// costo_item
		$this->costo_item->SetDbValueDef($rsnew, $this->costo_item->CurrentValue, NULL, strval($this->costo_item->CurrentValue) == "");

		// tipo_item
		$this->tipo_item->SetDbValueDef($rsnew, $this->tipo_item->CurrentValue, NULL, strval($this->tipo_item->CurrentValue) == "");

		// marca_item
		$this->marca_item->SetDbValueDef($rsnew, $this->marca_item->CurrentValue, NULL, FALSE);

		// cod_marca_item
		$this->cod_marca_item->SetDbValueDef($rsnew, $this->cod_marca_item->CurrentValue, NULL, FALSE);

		// detalle_item
		$this->detalle_item->SetDbValueDef($rsnew, $this->detalle_item->CurrentValue, NULL, FALSE);

		// saldo_item
		$this->saldo_item->SetDbValueDef($rsnew, $this->saldo_item->CurrentValue, NULL, FALSE);

		// activo_item
		$this->activo_item->SetDbValueDef($rsnew, $this->activo_item->CurrentValue, NULL, strval($this->activo_item->CurrentValue) == "");

		// maneja_serial_item
		$this->maneja_serial_item->SetDbValueDef($rsnew, $this->maneja_serial_item->CurrentValue, NULL, strval($this->maneja_serial_item->CurrentValue) == "");

		// asignado_item
		$this->asignado_item->SetDbValueDef($rsnew, $this->asignado_item->CurrentValue, NULL, strval($this->asignado_item->CurrentValue) == "");

		// si_no_item
		$this->si_no_item->SetDbValueDef($rsnew, $this->si_no_item->CurrentValue, NULL, strval($this->si_no_item->CurrentValue) == "");

		// precio_old_item
		$this->precio_old_item->SetDbValueDef($rsnew, $this->precio_old_item->CurrentValue, NULL, FALSE);

		// costo_old_item
		$this->costo_old_item->SetDbValueDef($rsnew, $this->costo_old_item->CurrentValue, NULL, FALSE);

		// registra_item
		$this->registra_item->SetDbValueDef($rsnew, $this->registra_item->CurrentValue, NULL, FALSE);

		// fecha_registro_item
		$this->fecha_registro_item->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fecha_registro_item->CurrentValue, 0), NULL, FALSE);

		// empresa_item
		$this->empresa_item->SetDbValueDef($rsnew, $this->empresa_item->CurrentValue, NULL, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$rsnew['Id_Item'] = $this->Id_Item->DbValue;
			}
		} else {
...
// Create page object
if (!isset($ap_items_inv_add)) $ap_items_inv_add = new cap_items_inv_add();

// Page init
$ap_items_inv_add->Page_Init();

// Page main
$ap_items_inv_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ap_items_inv_add->Page_Render();
?>
...
var CurrentForm = fap_items_invadd = new ew_Form("fap_items_invadd", "add");

// Validate form
fap_items_invadd.Validate = function() {
...
fap_items_invadd.Form_CustomValidate = 
...
fap_items_invadd.ValidateRequired = true;
<?php } else { ?>
fap_items_invadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$ap_items_inv_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $ap_items_inv_add->ShowPageHeader(); ?>
<?php
$ap_items_inv_add->ShowMessage();
?>
<form name="fap_items_invadd" id="fap_items_invadd" class="<?php echo $ap_items_inv_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($ap_items_inv_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $ap_items_inv_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $ap_items_inv_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fap_items_invadd.Init();
</script>
<?php
$ap_items_inv_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ap_items_inv_add->Page_Terminate();
?>
