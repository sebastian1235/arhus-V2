...
$ap_roles_usuarios_add = NULL; // Initialize page object first

class cap_roles_usuarios_add extends cap_roles_usuarios {
	...
	// Page object name
	var $PageObjName = 'ap_roles_usuarios_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["Id_Rol"] != "") {
				...
				$this->setKey("Id_Rol", $this->Id_Rol->CurrentValue); // Set up key
			} else {
				$this->setKey("Id_Rol", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "ap_roles_usuariosview.php")
	...
	function LoadDefaultValues() {
		$this->grupo_Rol->CurrentValue = NULL;
		$this->grupo_Rol->OldValue = $this->grupo_Rol->CurrentValue;
		$this->formulario_Rol->CurrentValue = NULL;
		$this->formulario_Rol->OldValue = $this->formulario_Rol->CurrentValue;
		$this->abrir_Rol->CurrentValue = 0;
		$this->agregar_Rol->CurrentValue = 0;
		$this->editar_Rol->CurrentValue = 0;
		$this->eliminar_Rol->CurrentValue = 0;
		$this->mostrar_Rol->CurrentValue = 0;
		$this->alias_Rol->CurrentValue = NULL;
		$this->alias_Rol->OldValue = $this->alias_Rol->CurrentValue;
		$this->empresa_Rol->CurrentValue = NULL;
		$this->empresa_Rol->OldValue = $this->empresa_Rol->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// grupo_Rol
		$this->grupo_Rol->SetDbValueDef($rsnew, $this->grupo_Rol->CurrentValue, NULL, FALSE);

		// formulario_Rol
		$this->formulario_Rol->SetDbValueDef($rsnew, $this->formulario_Rol->CurrentValue, "", FALSE);

		// abrir_Rol
		$this->abrir_Rol->SetDbValueDef($rsnew, $this->abrir_Rol->CurrentValue, NULL, strval($this->abrir_Rol->CurrentValue) == "");

		// agregar_Rol
		$this->agregar_Rol->SetDbValueDef($rsnew, $this->agregar_Rol->CurrentValue, NULL, strval($this->agregar_Rol->CurrentValue) == "");

		// editar_Rol
		$this->editar_Rol->SetDbValueDef($rsnew, $this->editar_Rol->CurrentValue, NULL, strval($this->editar_Rol->CurrentValue) == "");

		// eliminar_Rol
		$this->eliminar_Rol->SetDbValueDef($rsnew, $this->eliminar_Rol->CurrentValue, NULL, strval($this->eliminar_Rol->CurrentValue) == "");

		// mostrar_Rol
		$this->mostrar_Rol->SetDbValueDef($rsnew, $this->mostrar_Rol->CurrentValue, NULL, strval($this->mostrar_Rol->CurrentValue) == "");

		// alias_Rol
		$this->alias_Rol->SetDbValueDef($rsnew, $this->alias_Rol->CurrentValue, NULL, FALSE);

		// empresa_Rol
		$this->empresa_Rol->SetDbValueDef($rsnew, $this->empresa_Rol->CurrentValue, NULL, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->Id_Rol->setDbValue($conn->Insert_ID());
				$rsnew['Id_Rol'] = $this->Id_Rol->DbValue;
			}
		} else {
...
// Create page object
if (!isset($ap_roles_usuarios_add)) $ap_roles_usuarios_add = new cap_roles_usuarios_add();

// Page init
$ap_roles_usuarios_add->Page_Init();

// Page main
$ap_roles_usuarios_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ap_roles_usuarios_add->Page_Render();
?>
...
var CurrentForm = fap_roles_usuariosadd = new ew_Form("fap_roles_usuariosadd", "add");

// Validate form
fap_roles_usuariosadd.Validate = function() {
...
fap_roles_usuariosadd.Form_CustomValidate = 
...
fap_roles_usuariosadd.ValidateRequired = true;
<?php } else { ?>
fap_roles_usuariosadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$ap_roles_usuarios_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $ap_roles_usuarios_add->ShowPageHeader(); ?>
<?php
$ap_roles_usuarios_add->ShowMessage();
?>
<form name="fap_roles_usuariosadd" id="fap_roles_usuariosadd" class="<?php echo $ap_roles_usuarios_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($ap_roles_usuarios_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $ap_roles_usuarios_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $ap_roles_usuarios_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fap_roles_usuariosadd.Init();
</script>
<?php
$ap_roles_usuarios_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ap_roles_usuarios_add->Page_Terminate();
?>
