...
$ap_terceros_edit = NULL; // Initialize page object first

class cap_terceros_edit extends cap_terceros {
	...
	// Page object name
	var $PageObjName = 'ap_terceros_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->Id_tercero->CurrentValue == "") {
			$this->Page_Terminate("ap_terceroslist.php"); // Invalid key, return to list
		}
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->Id_tercero->FldIsDetailKey)
			$this->Id_tercero->setFormValue($objForm->GetValue("x_Id_tercero"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->Id_tercero->CurrentValue = $this->Id_tercero->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// nombre_tercero
			$this->nombre_tercero->SetDbValueDef($rsnew, $this->nombre_tercero->CurrentValue, NULL, $this->nombre_tercero->ReadOnly);

			// direccion_tercero
			$this->direccion_tercero->SetDbValueDef($rsnew, $this->direccion_tercero->CurrentValue, NULL, $this->direccion_tercero->ReadOnly);

			// telefono1_tercero
			$this->telefono1_tercero->SetDbValueDef($rsnew, $this->telefono1_tercero->CurrentValue, NULL, $this->telefono1_tercero->ReadOnly);

			// telefono2_tercero
			$this->telefono2_tercero->SetDbValueDef($rsnew, $this->telefono2_tercero->CurrentValue, NULL, $this->telefono2_tercero->ReadOnly);

			// fax_tercero
			$this->fax_tercero->SetDbValueDef($rsnew, $this->fax_tercero->CurrentValue, NULL, $this->fax_tercero->ReadOnly);

			// nit_tercero
			$this->nit_tercero->SetDbValueDef($rsnew, $this->nit_tercero->CurrentValue, NULL, $this->nit_tercero->ReadOnly);

			// tipo_tercero
			$this->tipo_tercero->SetDbValueDef($rsnew, $this->tipo_tercero->CurrentValue, NULL, $this->tipo_tercero->ReadOnly);

			// e_mail_tercero
			$this->e_mail_tercero->SetDbValueDef($rsnew, $this->e_mail_tercero->CurrentValue, NULL, $this->e_mail_tercero->ReadOnly);

			// Contacto_tercero
			$this->Contacto_tercero->SetDbValueDef($rsnew, $this->Contacto_tercero->CurrentValue, NULL, $this->Contacto_tercero->ReadOnly);

			// gran_contrib_tercero
			$this->gran_contrib_tercero->SetDbValueDef($rsnew, $this->gran_contrib_tercero->CurrentValue, NULL, $this->gran_contrib_tercero->ReadOnly);

			// autoretenedor_tercero
			$this->autoretenedor_tercero->SetDbValueDef($rsnew, $this->autoretenedor_tercero->CurrentValue, NULL, $this->autoretenedor_tercero->ReadOnly);

			// activo_tercero
			$this->activo_tercero->SetDbValueDef($rsnew, $this->activo_tercero->CurrentValue, NULL, $this->activo_tercero->ReadOnly);

			// tercero_ registrado_por
			$this->tercero__registrado_por->SetDbValueDef($rsnew, $this->tercero__registrado_por->CurrentValue, NULL, $this->tercero__registrado_por->ReadOnly);

			// reg_comun_tercero
			$this->reg_comun_tercero->SetDbValueDef($rsnew, $this->reg_comun_tercero->CurrentValue, NULL, $this->reg_comun_tercero->ReadOnly);

			// responsable_materiales_tercero
			$this->responsable_materiales_tercero->SetDbValueDef($rsnew, $this->responsable_materiales_tercero->CurrentValue, NULL, $this->responsable_materiales_tercero->ReadOnly);

			// grupo_nomina_tercero
			$this->grupo_nomina_tercero->SetDbValueDef($rsnew, $this->grupo_nomina_tercero->CurrentValue, NULL, $this->grupo_nomina_tercero->ReadOnly);

			// tercero_ lider_Obra
			$this->tercero__lider_Obra->SetDbValueDef($rsnew, $this->tercero__lider_Obra->CurrentValue, NULL, $this->tercero__lider_Obra->ReadOnly);

			// tercero_nombre_lider
			$this->tercero_nombre_lider->SetDbValueDef($rsnew, $this->tercero_nombre_lider->CurrentValue, NULL, $this->tercero_nombre_lider->ReadOnly);

			// empresa_tercero
			$this->empresa_tercero->SetDbValueDef($rsnew, $this->empresa_tercero->CurrentValue, NULL, $this->empresa_tercero->ReadOnly);
...
// Create page object
if (!isset($ap_terceros_edit)) $ap_terceros_edit = new cap_terceros_edit();

// Page init
$ap_terceros_edit->Page_Init();

// Page main
$ap_terceros_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ap_terceros_edit->Page_Render();
?>
...
var CurrentForm = fap_tercerosedit = new ew_Form("fap_tercerosedit", "edit");

// Validate form
fap_tercerosedit.Validate = function() {
...
fap_tercerosedit.Form_CustomValidate = 
...
fap_tercerosedit.ValidateRequired = true;
<?php } else { ?>
fap_tercerosedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$ap_terceros_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $ap_terceros_edit->ShowPageHeader(); ?>
<?php
$ap_terceros_edit->ShowMessage();
?>
<form name="fap_tercerosedit" id="fap_tercerosedit" class="<?php echo $ap_terceros_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($ap_terceros_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $ap_terceros_edit->Token ?>">
<?php } ?>
...
<?php if ($ap_terceros->Id_tercero->Visible) { // Id_tercero ?>
	<div id="r_Id_tercero" class="form-group">
		<label id="elh_ap_terceros_Id_tercero" class="col-sm-2 control-label ewLabel"><?php echo $ap_terceros->Id_tercero->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $ap_terceros->Id_tercero->CellAttributes() ?>>
<span id="el_ap_terceros_Id_tercero">
<span<?php echo $ap_terceros->Id_tercero->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $ap_terceros->Id_tercero->EditValue ?></p></span>
</span>
<input type="hidden" data-table="ap_terceros" data-field="x_Id_tercero" name="x_Id_tercero" id="x_Id_tercero" value="<?php echo ew_HtmlEncode($ap_terceros->Id_tercero->CurrentValue) ?>">
<?php echo $ap_terceros->Id_tercero->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $ap_terceros_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fap_tercerosedit.Init();
</script>
<?php
$ap_terceros_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ap_terceros_edit->Page_Terminate();
?>
