...
function CurrentProjectID() {
	...
	return "{6A9AC718-A824-45A3-BED3-A95E91E6986D}";
}
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($validateUser) {
						...
						$this->setSessionParentUserID($rs->fields('rol')); // Load parent User ID
	...
	function ParentUserIDList($userid) {
		...
		if (trim(strval($userid)) == strval(CurrentUserID())) {
			if (strval(CurrentParentUserID()) <> "")
				$result = ew_QuotedValue(CurrentParentUserID(), EW_DATATYPE_NUMBER, EW_USER_TABLE_DBID);
			return $result;
		}

		// One level only, must be CurrentUserID
		if (!EW_USER_ID_IS_HIERARCHICAL) {
			return ew_QuotedValue(CurrentUserID(), EW_DATATYPE_NUMBER, EW_USER_TABLE_DBID);
			...
			for ($i = 0; $i < $len; $i++) {
				if (strval($ar[$i]) <> trim(strval($userid))) {
					if ($result <> "")
						$result .= ", ";
					$result .= ew_QuotedValue($ar[$i], EW_DATATYPE_NUMBER, EW_USER_TABLE_DBID);
				}
			}
			return $result;
		}
