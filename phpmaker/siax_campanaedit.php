...
$siax_campana_edit = NULL; // Initialize page object first

class csiax_campana_edit extends csiax_campana {
	...
	// Page object name
	var $PageObjName = 'siax_campana_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->id_campana->CurrentValue == "") {
			$this->Page_Terminate("siax_campanalist.php"); // Invalid key, return to list
		}
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->id_campana->FldIsDetailKey)
			$this->id_campana->setFormValue($objForm->GetValue("x_id_campana"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->id_campana->CurrentValue = $this->id_campana->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// nombre_campana
			$this->nombre_campana->SetDbValueDef($rsnew, $this->nombre_campana->CurrentValue, "", $this->nombre_campana->ReadOnly);

			// descuente_campana
			$this->descuente_campana->SetDbValueDef($rsnew, $this->descuente_campana->CurrentValue, NULL, $this->descuente_campana->ReadOnly);

			// desc_financ_campana
			$this->desc_financ_campana->SetDbValueDef($rsnew, $this->desc_financ_campana->CurrentValue, NULL, $this->desc_financ_campana->ReadOnly);

			// plazo_max_campana
			$this->plazo_max_campana->SetDbValueDef($rsnew, $this->plazo_max_campana->CurrentValue, NULL, $this->plazo_max_campana->ReadOnly);

			// detalle_campana
			$this->detalle_campana->SetDbValueDef($rsnew, $this->detalle_campana->CurrentValue, NULL, $this->detalle_campana->ReadOnly);

			// aplicacion_campana
			$this->aplicacion_campana->SetDbValueDef($rsnew, $this->aplicacion_campana->CurrentValue, NULL, $this->aplicacion_campana->ReadOnly);

			// desde_campana
			$this->desde_campana->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->desde_campana->CurrentValue, 0), NULL, $this->desde_campana->ReadOnly);

			// hasta_campana
			$this->hasta_campana->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->hasta_campana->CurrentValue, 0), NULL, $this->hasta_campana->ReadOnly);

			// vigente_campana
			$this->vigente_campana->SetDbValueDef($rsnew, $this->vigente_campana->CurrentValue, NULL, $this->vigente_campana->ReadOnly);

			// tasa_campana
			$this->tasa_campana->SetDbValueDef($rsnew, $this->tasa_campana->CurrentValue, NULL, $this->tasa_campana->ReadOnly);

			// descuento_fijo_campana
			$this->descuento_fijo_campana->SetDbValueDef($rsnew, $this->descuento_fijo_campana->CurrentValue, NULL, $this->descuento_fijo_campana->ReadOnly);

			// manto_max_campana
			$this->manto_max_campana->SetDbValueDef($rsnew, $this->manto_max_campana->CurrentValue, NULL, $this->manto_max_campana->ReadOnly);

			// condiciones_campana
			$this->condiciones_campana->SetDbValueDef($rsnew, $this->condiciones_campana->CurrentValue, NULL, $this->condiciones_campana->ReadOnly);
...
// Create page object
if (!isset($siax_campana_edit)) $siax_campana_edit = new csiax_campana_edit();

// Page init
$siax_campana_edit->Page_Init();

// Page main
$siax_campana_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$siax_campana_edit->Page_Render();
?>
...
var CurrentForm = fsiax_campanaedit = new ew_Form("fsiax_campanaedit", "edit");

// Validate form
fsiax_campanaedit.Validate = function() {
...
fsiax_campanaedit.Form_CustomValidate = 
...
fsiax_campanaedit.ValidateRequired = true;
<?php } else { ?>
fsiax_campanaedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$siax_campana_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $siax_campana_edit->ShowPageHeader(); ?>
<?php
$siax_campana_edit->ShowMessage();
?>
<form name="fsiax_campanaedit" id="fsiax_campanaedit" class="<?php echo $siax_campana_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($siax_campana_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $siax_campana_edit->Token ?>">
<?php } ?>
...
<?php if ($siax_campana->id_campana->Visible) { // id_campana ?>
	<div id="r_id_campana" class="form-group">
		<label id="elh_siax_campana_id_campana" class="col-sm-2 control-label ewLabel"><?php echo $siax_campana->id_campana->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $siax_campana->id_campana->CellAttributes() ?>>
<span id="el_siax_campana_id_campana">
<span<?php echo $siax_campana->id_campana->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $siax_campana->id_campana->EditValue ?></p></span>
</span>
<input type="hidden" data-table="siax_campana" data-field="x_id_campana" name="x_id_campana" id="x_id_campana" value="<?php echo ew_HtmlEncode($siax_campana->id_campana->CurrentValue) ?>">
<?php echo $siax_campana->id_campana->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $siax_campana_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fsiax_campanaedit.Init();
</script>
<?php
$siax_campana_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$siax_campana_edit->Page_Terminate();
?>
