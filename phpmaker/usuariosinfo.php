...
class cusuarios extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('usuarios', 'usuarios', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// password
		$this->password = new cField('usuarios', 'usuarios', 'x_password', 'password', '`password`', '`password`', 201, -1, FALSE, '`password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->password->Sortable = TRUE; // Allow sort
		$this->fields['password'] = &$this->password;

		// email
		$this->_email = new cField('usuarios', 'usuarios', 'x__email', 'email', '`email`', '`email`', 201, -1, FALSE, '`email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->_email->Sortable = TRUE; // Allow sort
		$this->fields['email'] = &$this->_email;

		// photo
		$this->photo = new cField('usuarios', 'usuarios', 'x_photo', 'photo', '`photo`', '`photo`', 201, -1, FALSE, '`photo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->photo->Sortable = TRUE; // Allow sort
		$this->fields['photo'] = &$this->photo;

		// rol
		$this->rol = new cField('usuarios', 'usuarios', 'x_rol', 'rol', '`rol`', '`rol`', 3, -1, FALSE, '`rol`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->rol->Sortable = TRUE; // Allow sort
		$this->rol->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->rol->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->rol->OptionCount = 7;
		$this->rol->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['rol'] = &$this->rol;

		// intentos
		$this->intentos = new cField('usuarios', 'usuarios', 'x_intentos', 'intentos', '`intentos`', '`intentos`', 3, -1, FALSE, '`intentos`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->intentos->Sortable = TRUE; // Allow sort
		$this->intentos->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['intentos'] = &$this->intentos;

		// Activo
		$this->Activo = new cField('usuarios', 'usuarios', 'x_Activo', 'Activo', '`Activo`', '`Activo`', 16, -1, FALSE, '`Activo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Perfil
		$this->Perfil = new cField('usuarios', 'usuarios', 'x_Perfil', 'Perfil', '`Perfil`', '`Perfil`', 201, -1, FALSE, '`Perfil`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->Perfil->EditValue = $this->Perfil->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Perfil->Exportable) $Doc->ExportCaption($this->Perfil);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->usuario->Exportable) $Doc->ExportCaption($this->usuario);
					if ($this->rol->Exportable) $Doc->ExportCaption($this->rol);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->Perfil->Exportable) $Doc->ExportField($this->Perfil);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->usuario->Exportable) $Doc->ExportField($this->usuario);
						if ($this->rol->Exportable) $Doc->ExportField($this->rol);
	...
	function UserIDFilter($userid) {
		...
		$sParentUserIDFilter = '`id` IN (SELECT `id` FROM ' . "`usuarios`" . ' WHERE `rol` = ' . ew_QuotedValue($userid, EW_DATATYPE_NUMBER, EW_USER_TABLE_DBID) . ')';
